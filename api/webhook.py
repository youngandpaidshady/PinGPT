...
def build_lasttrain_instruction(character_name, count):
    ...
    return (
        ...
        f"- SUIT ENFORCEMENT: Character MUST wear a dark suit, dress shirt, or formal overcoat in EVERY prompt. "
        f"NO casual jackets, NO hoodies, NO sweaters, NO streetwear. This is a SUITED aesthetic. "
        f"Even relaxed variants must be suit-based (e.g. suit jacket removed but vest/dress shirt visible, "
        f"or overcoat over suit). The outfit parameter above is your guide.\n"
        f"- PHONE POSE CAP: Maximum 2 prompts in a batch may feature the character holding/looking at a phone. "
        f"Remaining prompts MUST use hands-free narrative poses (leaning on glass, tie pull, head bowed, "
        f"hands in pockets, gripping pole, briefcase between feet).\n"
